...
class Wallet(Client):
    ...
    def request_wallet_instance_attestation(self, wallet_provider_id, challenge, ephemeral_key_tag):
        ...
        # # Store the request
        # _assertion = JWT(self.context.keyjar, sign_alg="ES256")
        # _assertion.iss = self.entity_id
        # _wia_flow["request_assertion"] = _assertion.pack(payload=war_payload, kid=ephemeral_key_tag)
        ...
        return wallet_instance_attestation, war_payload

    def get_ephemeral_key(self, ephemeral_key_tag):
        return self.context.ephemeral_key[ephemeral_key_tag]